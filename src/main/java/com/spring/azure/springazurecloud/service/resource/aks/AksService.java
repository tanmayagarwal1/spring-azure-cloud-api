...
public class AksService {
    ...
    private final AksValidationService aksValidationService;

    /* Aks Cluster Methods */

    public AksCluster getAksCluster(AksRequestDto dto, String username){
        if (!aksValidationService.validateExistsAks(dto.getAksId(), username))throw new AksException("Cluster not Found");
    ...
    public NodePool getNodePool(AksRequestDto dto,String username){
        if (!aksValidationService.validateExistsAks(dto.getAksId(), username))throw new AksException("Cluster not Found");
        if(!aksValidationService.validateExistsResource(dto.getResourceId(), NodePool.class)) throw new AksException("NodePool not Found");
        return transactionTemplate.execute(status -> sessionFactory
                .getCurrentSession()
                .createQuery(
    ...
    public String createNodePool(NodePool nodePool, String username){
        String aksId =nodePool.getAksCluster() != null ?
                ...
                : null;
        if(aksId == null || !aksValidationService.validateExistsResource(aksId, AksCluster.class))
            throw new AksException("No Aks with id "+aksId+" Found");
        AtomicReference<String> id = new AtomicReference<>(Constants.PLACEHOLDER);
        transactionTemplate.execute(status -> {
           if(!aksValidationService.validateExistsAks(nodePool.getAksCluster().getResourceId(), username)) throw new AksException("Aks Cluster not found");
    ...
    public Node getNode(AksRequestDto dto, String username){
        if (!aksValidationService.validateExistsAks(dto.getAksId(), username))throw new AksException("Cluster not Found");
        if(!aksValidationService.validateExistsResource(dto.getResourceId(), Node.class)) throw new AksException("Node not Found");
        return transactionTemplate.execute(status -> sessionFactory
                .getCurrentSession()
                .createQuery(
    ...
    public String createNode(Node node, String username){
        String nodePoolId = node.getNodePool() != null ?
                ...
                : null;
        if(nodePoolId == null || !aksValidationService.validateExistsResource(nodePoolId, NodePool.class))
    ...
    public Pod getPod(AksRequestDto dto, String username){
        if (!aksValidationService.validateExistsAks(dto.getAksId(), username))throw new AksException("Cluster not Found");
        if(!aksValidationService.validateExistsResource(dto.getResourceId(), Pod.class)) throw new AksException("Pod not Found");
        return transactionTemplate.execute(status -> sessionFactory
                .getCurrentSession()
                .createQuery(
    ...
    public String createPod(Pod pod, String username){
        String nodeId = pod.getNode() != null ?
                ...
                : null ;
        if(nodeId == null || !aksValidationService.validateExistsResource(nodeId, Node.class))
    ...
    public Container getContainer(AksRequestDto dto, String username){
        if (!aksValidationService.validateExistsAks(dto.getAksId(), username))throw new AksException("Cluster not Found");
        if(!aksValidationService.validateExistsResource(dto.getResourceId(), Container.class)) throw new AksException("Container not Found");
        return transactionTemplate.execute(status -> sessionFactory
                .getCurrentSession()
                .createQuery(
    ...
    public String createContainer(Container container, String username){
        String podId = container.getPod() != null ?
                ...
                : null;
        if(podId == null || !aksValidationService.validateExistsResource(podId, Pod.class))
    ...
    public<T extends Resource> void deleteAksResource(AksRequestDto dto, String username, Class<T> clazz){
        if (!aksValidationService.validateExistsAks(dto.getAksId(), username))throw new AksException("Cluster not Found");
        if(!aksValidationService.validateExistsResource(dto.getResourceId(), clazz))
