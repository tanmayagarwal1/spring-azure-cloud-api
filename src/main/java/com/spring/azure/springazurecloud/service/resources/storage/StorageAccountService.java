package com.spring.azure.springazurecloud.service.resources.storage;
...
public class StorageAccountService {
    ...
    public StorageAccount getStorageAccount(StorageAccountRequestDto dto, String username){
        if (!validationService.validateExistsMainResource(StorageAccount.class, dto.getStorageAccountId(), username))
    ...
    public String createContainer(StorageContainer storageContainer, String username){
        ...
        AtomicReference<String> id = new AtomicReference<>(Constants.PLACEHOLDER);
        transactionTemplate.execute(status -> {
            if(!validationService.validateExistsMainResource(StorageAccount.class, storageContainer.getStorageAccount().getResourceId(), username))
    ...
    public<T extends Resource> void deleteStorageAccountResource(StorageAccountRequestDto dto, String username, Class<T> clazz){
        if (!validationService.validateExistsMainResource(StorageAccount.class, dto.getStorageAccountId(), username))throw new StorageAccountException("Storage Account not Found");
